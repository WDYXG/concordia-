...
interface MemoryRecord {
  memory_id: string;
  agent_id: string;
  memory_type: "episodic" | "semantic" | "social";
  content: string;
  round_index: number;
...
interface RunMetrics {
  ...
  unvoted_agent_ids?: string[];
  candidate_tally: Record<string, number>;
...
interface RunSource {
  ...
  run_id?: string;
  run_at?: string;
  controller_traces?: Record<string, ControllerTrace>;
...
interface DemoBundle {
  ...
  experiment_plan: {
    base_seed: number;
    repetitions_per_condition: number;
    runs: JsonMap[];
  };
  experiment_summary: {
    unit_note: string;
    conditions: Record<
      string,
      {
        ...
        all_manipulation_checks_passed: boolean;
...
const conditionSelect = required<HTMLSelectElement>("condition-select");
const timeline = required<HTMLInputElement>("timeline");
const speedSelect = required<HTMLSelectElement>("speed-select");
const agentList = required<HTMLDivElement>("agent-list");
const eventFeed = required<HTMLOListElement>("event-feed");
const agentDetail = required<HTMLDivElement>("agent-detail");
const importInput = required<HTMLInputElement>("import-input");
const importButton = required<HTMLButtonElement>("import-button");
const demoButton = required<HTMLButtonElement>("demo-button");

let bundle: DemoBundle;
let demoBundle: DemoBundle;
let selectedCondition = 0;
let frameIndex = 0;
let selectedAgentId = "";
let timer: number | null = null;
let renderedAgentPoints: Array<{
  agentId: string;
  x: number;
  y: number;
}> = [];
...
const condition = (): ConditionBundle =>
  bundle.conditions[selectedCondition];

const run = (): SimulationRun => condition().run;

const timeLabel = (index: number): string =>
  run().config.metadata.time_unit === "day"
    ? `第 ${index} 天`
    : `第 ${index} 轮`;
...
const agentById = (agentId: string): AgentSpec =>
  run().scenario.agents.find((agent) => agent.agent_id === agentId) ??
  run().scenario.agents[0];

const locationById = (locationId: string): LocationSpec =>
  run().scenario.locations.find(
    (location) => location.location_id === locationId,
  ) ?? run().scenario.locations[0];

const agentColor = (agentId: string): string => {
  const index = run().scenario.agents.findIndex(
...
const actionSummary = (turn: TurnRecord | null): string => {
  if (!turn) {
    return "世界已初始化";
  }
  ...
  if (!turn.result) {
    const period =
      run().config.metadata.time_unit === "day" ? "本日" : "本轮";
...
const voteRecords = (): VoteRecord[] =>
  run().turns.flatMap((turn, turnIndex) => {
    const result = turn.result;
    if (
      !result?.accepted ||
      result.request.action_type !== "vote"
    ) {
      return [];
    }
    ...
    return [
      {
        ...
        candidate: String(
          result.request.parameters.candidate ?? "未记录",
        ),
        reason:
...
const allVisibleEvents = (): WorldEvent[] => currentState().events;
...
const drawWorld = (): void => {
  ...
  for (const location of run().scenario.locations) {
    const [normalizedX, normalizedY] =
      locationPositions[location.location_id] ?? [0.5, 0.5];
    ...
    context.fillStyle =
      location.location_id === "factory_district"
        ? "#8f8178"
        : location.location_id === "town_hall"
          ? "#d7c6a1"
          : "#f7f7f1";
    ...
    context.font =
      '600 11px "Segoe UI", "Microsoft YaHei", sans-serif';
    context.fillText(location.name, x, y);
  }

  const state = currentState();
  const groups = new Map<string, string[]>();
  for (const [agentId, locationId] of Object.entries(
  ...
  for (const [locationId, agentIds] of groups) {
    ...
    agentIds.forEach((agentId, index) => {
      ...
      if (agentId === selectedAgentId || agentId === activeAgent) {
        ...
        context.fillStyle =
          agentId === activeAgent ? "#fff1b8" : "#ffffff";
        context.fill();
...
const renderAgentList = (): void => {
  const state = currentState();
  agentList.replaceChildren(
    ...run().scenario.agents.map((agent) => {
      const button = document.createElement("button");
      button.type = "button";
      button.className = `agent-row${
        agent.agent_id === selectedAgentId ? " selected" : ""
      }`;
      button.innerHTML = `
...
const renderEventFeed = (): void => {
  const events = allVisibleEvents();
  const votesByEventId = new Map(
    voteRecords().map((record) => [record.eventId, record]),
  );
  const activeEventId =
    currentTurn()?.result?.events[0]?.event_id ?? "";
  eventFeed.replaceChildren(
    ...events
      ...
      .map((event) => {
        ...
        item.className = `event-item${event.is_public ? "" : " private"}${
          event.event_id === activeEventId ? " active" : ""
        }`;
        const actor = event.actor_id
          ? agentById(event.actor_id).name
          : "Game Master";
...
const renderVoteDetail = (): void => {
  const container = required<HTMLDivElement>("vote-detail");
  ...
  const entries = records
    .map((record) => {
      const candidateClass =
        record.candidate === "Alice"
          ? "alice"
          : record.candidate === "Bob"
            ? "bob"
            : "unknown";
      const activeClass =
        activeTurn === record.turn ? " active" : "";
      return `
...
const renderAgentDetail = (): void => {
  ...
  const memories = (
    run().memory_state[agent.agent_id]?.records ?? []
  )
    .filter((memory) => memory.round_index <= currentRound)
    .slice(-6)
    .reverse();
...
const renderModelDetail = (): void => {
  const container = required<HTMLDivElement>("model-detail");
  const source = condition().source;
  const usage = source?.model_usage;
  const turn = currentTurn();
  const modelName =
    usage?.model ?? run().config.model_name ?? "无模型调用";
  const summary = `
  ...
  const calls = (trace?.calls ?? []).filter(
    (call) => call.round_index === turn.round_index,
  );
  const result = turn.result;
    ...
    .map(
      (call, index) => `
        <div class="trace-block">
          ...
          ${
            call.error
              ? `<p class="trace-status rejected">${escapeHtml(call.error)}</p>`
              : ""
          }
...
const renderConditionBars = (): void => {
  const container = required<HTMLDivElement>("condition-bars");
  container.replaceChildren(
    ...bundle.conditions.map((item) => {
      ...
      const eligibleVoters =
        item.metrics.eligible_voters ??
        item.run.scenario.agents.length;
      const unvoted =
        item.metrics.unvoted_count ??
        Math.max(0, eligibleVoters - totalVotes);
...
const renderMetrics = (): void => {
  ...
  required("accepted-actions").textContent = String(
    metrics.accepted_actions,
  );
  required("relationship-edges").textContent = String(
    metrics.relationship_edges,
  );
  required("manipulation-check").textContent =
    metrics.manipulation_check_passed ? "通过" : "未通过";
  const validation = required("validation-status");
  validation.textContent = metrics.manipulation_check_passed
    ? "操纵检查通过"
    : "操纵检查失败";
  validation.classList.toggle(
    "pass",
    metrics.manipulation_check_passed,
  );
};
...
const render = (): void => {
  ...
  required("step-label").textContent =
    `步骤 ${frameIndex}/${run().turns.length}`;
...
const switchCondition = (index: number): void => {
  stopPlayback();
...
const showTab = (
  ...
  required("events-tab").setAttribute(
    "aria-selected",
    String(eventsSelected),
  );
  required("agent-tab").setAttribute(
    "aria-selected",
    String(agentSelected),
  );
  required("model-tab").setAttribute(
    "aria-selected",
    String(modelSelected),
  );
  required("vote-tab").setAttribute(
    "aria-selected",
    String(voteSelected),
  );
...
const validateLivePayload = (
  ...
  if (
    ...
    !isRecord(runValue) ||
    !isRecord(metricsValue) ||
    !isRecord(tracesValue) ||
    !isRecord(usageValue)
  ) {
  ...
  if (
    ...
    !Array.isArray(scenario.locations) ||
    scenario.locations.length === 0 ||
    !Array.isArray(runValue.turns) ||
    runValue.turns.length === 0 ||
    !Array.isArray(runValue.snapshots) ||
    runValue.snapshots.length === 0 ||
    !isRecord(runValue.final_state) ||
    !isRecord(runValue.memory_state)
  ) {
  ...
  if (
    ...
    metricsValue.condition !== config.condition ||
    !isRecord(metricsValue.candidate_tally)
  ) {
    throw new Error(`${fileName}: 实验条件或指标不一致。`);
  }
  if (
    typeof usageValue.model !== "string" ||
    typeof usageValue.request_count !== "number" ||
    typeof usageValue.total_tokens !== "number"
  ) {
    throw new Error(`${fileName}: 缺少模型或 token 用量记录。`);
  }
  return value as unknown as LiveRunPayload;
};

const buildLiveBundle = (
  entries: Array<{ fileName: string; payload: LiveRunPayload }>,
): DemoBundle => {
  const ordered = entries.slice().sort((left, right) =>
    left.payload.run_at.localeCompare(right.payload.run_at),
  );
  const usedNames = new Set<string>();
  const conditions: ConditionBundle[] = ordered.map(
    ({ fileName, payload }, index) => {
      ...
      return {
        name: uniqueName,
        label:
          `API · ${conditionLabels[conditionName] ?? conditionName}` +
          `${time ? ` · ${time}` : ` · 运行 ${index + 1}`}`,
        run: payload.run,
  ...
  const summaries: DemoBundle["experiment_summary"]["conditions"] = {};
  for (const item of conditions) {
    ...
    summaries[item.name] = {
      ...
      all_manipulation_checks_passed:
        item.metrics.manipulation_check_passed,
      mean_acceptance_rate:
        item.metrics.acceptance_rate ??
        item.metrics.accepted_actions /
          Math.max(
            1,
            item.metrics.accepted_actions +
              item.metrics.rejected_actions,
          ),
    };
  }
  return {
    generated_at:
      ordered.at(-1)?.payload.run_at ?? new Date().toISOString(),
    mode: "live_api_import",
    disclaimer:
      "这些数据来自本机保存的 DeepSeek API 实验结果；网页只做本地读取和回放，不会再次调用 API 或上传文件。",
    conditions,
    experiment_plan: {
...
const applyBundle = (nextBundle: DemoBundle): void => {
  stopPlayback();
  bundle = nextBundle;
  selectedCondition = 0;
  frameIndex = 0;
  conditionSelect.replaceChildren(
    ...bundle.conditions.map((item, index) => {
  ...
  selectedAgentId = run().scenario.agents[0].agent_id;
  timeline.max = String(run().turns.length);
  required("agent-count").textContent = String(
    run().scenario.agents.length,
  );
  const liveMode = bundle.mode === "live_api_import";
  required("run-mode").textContent = liveMode
    ? "DeepSeek API 实验回放"
    : "确定性多 Agent 社会模拟";
  required("experiment-title").textContent = liveMode
    ? "已导入的 API 实验"
    : "四组演示结果";
  required("experiment-subtitle").textContent = liveMode
    ? "真实模型行动、GM 判定与最终选票"
    : "脚本数据仅用于验证流程";
  required("data-disclaimer").textContent = bundle.disclaimer;
  required("data-time").textContent = formatRunTime(
    bundle.generated_at,
  );
  if (liveMode) {
    const usages = bundle.conditions
      .map((item) => item.source?.model_usage)
      .filter((usage): usage is ModelUsage => Boolean(usage));
    const requests = usages.reduce(
      (total, usage) => total + usage.request_count,
      0,
    );
    ...
    required("plan-summary").textContent =
      `${bundle.conditions.length} 个 API 运行 · ` +
      `${formatInteger(requests)} 次请求 · ` +
      `${formatInteger(tokens)} tokens`;
  } else {
    required("plan-summary").textContent =
      `${bundle.experiment_plan.runs.length} 个计划运行 · ` +
      `种子 ${bundle.experiment_plan.base_seed} · 顺序已平衡`;
  }
...
const importLiveRuns = async (files: FileList): Promise<void> => {
  if (!files.length) {
    return;
  }
...
const bindControls = (): void => {
  ...
  importInput.addEventListener("change", () => {
    if (importInput.files) {
      void importLiveRuns(importInput.files);
    }
  });
  ...
  required("events-tab").addEventListener("click", () =>
    showTab("events"),
  );
  required("agent-tab").addEventListener("click", () =>
    showTab("agent"),
  );
  required("model-tab").addEventListener("click", () =>
    showTab("model"),
  );
  required("vote-tab").addEventListener("click", () =>
    showTab("vote"),
  );
  canvas.addEventListener("click", (event) => {
    ...
    const nearest = renderedAgentPoints
      .map((point) => ({
        ...point,
...
const initialize = async (): Promise<void> => {
  ...
  if (embedded?.textContent) {
    bundle = JSON.parse(embedded.textContent) as DemoBundle;
  } else {
    ...
    bundle = (await response.json()) as DemoBundle;
  }

  demoBundle = {
    ...bundle,
    conditions: bundle.conditions.map((condition) => ({
      ...condition,
      source: condition.source ?? { kind: "demo" },
    })),
  };
  bindControls();
